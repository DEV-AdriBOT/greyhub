...
export default async function HistoryPage({
  ...
  if (canViewCompany) {
    ...
    if (view === "completed")
      clauses.push("orders.status IN ('completed', 'cancelled')");
    if (view === "abandoned")
      clauses.push(
        "EXISTS (SELECT 1 FROM order_workers abandoned WHERE abandoned.order_id = orders.id AND abandoned.abandoned_at IS NOT NULL)",
      );
    if (query.worker && Number(query.worker)) {
      clauses.push(
        "EXISTS (SELECT 1 FROM order_workers filtered_worker WHERE filtered_worker.order_id = orders.id AND filtered_worker.user_id = ?)",
      );
      values.push(Number(query.worker));
    }
  } else {
    clauses.push(
      "EXISTS (SELECT 1 FROM order_workers mine WHERE mine.order_id = orders.id AND mine.user_id = ? AND mine.abandoned_at IS " +
        (view === "abandoned" ? "NOT NULL" : "NULL") +
        ")",
    );
    values.push(user.id);
    if (view === "current")
      clauses.push(
        "orders.status IN ('claimed', 'in_progress', 'pending_review')",
      );
  ...
  if (query.from) {
    clauses.push("date(orders.created_at) >= date(?)");
    ...
    <>
      ...
      <nav className="tab-bar">
        <Link
          ...
          href="/history?view=current"
        >
          Current
        </Link>
        <Link
          className={view === "completed" ? "active" : ""}
          href="/history?view=completed"
        >
          Completed
        </Link>
        <Link
          className={view === "abandoned" ? "active" : ""}
          href="/history?view=abandoned"
        >
          Abandoned
        </Link>
      </nav>
      ...
      <section className="orders-board">
          ...
          <table>
            <thead>
              <tr>
                <th>Order</th>
                <th>Client</th>
                <th>{view === "abandoned" ? "Abandoned by" : "Workers"}</th>
                <th>Date</th>
                <th>Reward</th>
                <th>Status</th>
                <th>Payment</th>
            ...
            <tbody>
              {orders.map((order) => (
                <tr key={order.id}>
                  ...
                  <td>
                    {view === "abandoned"
                      ? order.abandoned_workers || "—"
                      : order.workers || "—"}
                  </td>
                  <td>
                    {shortDate(order.completion_date || order.created_at)}
                  </td>
        ...
        {!orders.length && (
          <div className="empty-state">Nothing has been filed here yet.</div>
        )}
      </section>
    </>
  );
