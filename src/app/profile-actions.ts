...
export async function updateProfileImage(formData: FormData) {
  ...
  if (!(file instanceof File) || file.size === 0) redirect("/profile?error=Choose+an+image");
  const allowed: Record<string, string> = { "image/jpeg": ".jpg", "image/png": ".png", "image/webp": ".webp" };
  if (!allowed[file.type] || file.size > 3 * 1024 * 1024) redirect("/profile?error=Use+a+JPG,+PNG+or+WebP+under+3MB");

  const uploadDir = path.join(process.cwd(), "public", "uploads", "profiles");
  await fs.mkdir(uploadDir, { recursive: true });
  const filename = `${crypto.randomUUID()}${allowed[file.type]}`;
  await fs.writeFile(path.join(uploadDir, filename), Buffer.from(await file.arrayBuffer()));
  db.prepare("UPDATE users SET profile_image = ? WHERE id = ?").run(`/uploads/profiles/${filename}`, user.id);
